...
exports.getById = (req, res) => {
  ...
  db.query(`select * from dev.users where user_id=$0 and is_active=true`,[user_id])
    .then((data) => {
      res.status(200).json(data.rows);
    })
    .catch((err) => {
...
try {
  ...
  const encryptedPassword = await bcrypt.hash(user.password, 5);
  ...
  await db.query('INSERT INTO dev.login_details (username, password_hash) VALUES ($1, $2)', [rows[0].username, encryptedPassword]);
...
exports.delete = (req, res) => {
  const id = req.params.id;
  db.query('update dev.users set is_active=false where user_id=$1 RETURNING *',[id])
    .then((data) => { 
      if (data?.rows?.length > 0) {
        res.send({
